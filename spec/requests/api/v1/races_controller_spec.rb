    ...
    describe "RESTful endpoints" do
        ...
        describe "GET /api/v1/races/:id" do
            ...
            describe "sad paths" do
                before do
                    stub_request(:get, "https://www.dnd5eapi.co/api/2014/races/test").to_return(status: 404, body: { error: "Not found" }.to_json)
                end

                it "returns a 404 status when target race does not exist" do
                    get "/api/v1/races/test"

                    expect(response).to have_http_status(:not_found)
                    expect(JSON.parse(response.body)).to include("error" => "Race not found")
                end
            end
        end
    end
